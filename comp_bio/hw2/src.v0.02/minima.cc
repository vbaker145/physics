...
void minima::steepest_desdents_arbitrary_step_one_particle(int part, double step_size, double crit, forces &physics, ensemble& sys)
{
  ...
  while (keep_going)
  {
		curr_energy = physics.calc_LJ_forces_single_particle(part, direction, sys);
		...
		new_direction     = sys.atoms[part].r + step_size * direction;
		sys.atoms[part].r_old  = sys.atoms[part].r;
		sys.atoms[part].r      = new_direction;
...
void minima::steepest_desdents_arbitrary_step(double step_size, double crit, forces &physics, ensemble& sys)
{
  ...
  for (part=0; part<sys.N; part++)
  {
	...
	// PRINT PROGRESS
	PRINT3("MINIMIZING AT",100.*double(part+1)/double(sys.N),"%");
