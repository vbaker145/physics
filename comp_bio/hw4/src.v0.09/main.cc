...
int mainFunc(int argc, char* argv[], double rl)
{
  // =============================================
  ...
  //======= VJB: Override file parameters for multi-param runs ====
  sys.rl = rl;
...
int main(int argc, char* argv[] ) {
	...
	for( double rl=2.6; rl < 4.5; rl += 0.1) {
		t1 = clock();
		mainFunc(argc, argv, rl);
		t2 = clock();
		delTime = ((float)t2-(float)t1)/CLOCKS_PER_SEC;
		cout << rl << "\t" << delTime << endl;
	}

	for( double rl=4.5; rl <= 6.5; rl += 0.5) {
			t1 = clock();
			mainFunc(argc, argv, rl);
			t2 = clock();
			delTime = ((float)t2-(float)t1)/CLOCKS_PER_SEC;
