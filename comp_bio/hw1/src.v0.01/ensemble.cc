...
void ensemble::readFile(char* the_file)
{
  ...
  if (density_orig != density)
	cout << " * * WARNING: Requested density could not be met. New: " 
	  << density
	  << " requested: "
	  << density_orig
	  << " where abs(new-old) = "
  ...
  // TIME STEP
  total_num_steps = ANINT( double(total_time) / delta_t );
...
void ensemble::init(int a,int b)
{
  ...
  switch(b)
  {
	case 1:
	  ...
	  break;
	default:
	  cout << "WARNING: no velocity initialization given " << a << "\n";
  }
}
...
void ensemble::init_random_pos_check_energy(void)
{
  ...
  //double threshold_energy = 1.7;
  // GOOD FOR 3D
  //double threshold_energy = 3.3;
  //double threshold_energy = 2.3;
  double threshold_energy = 1.0;
  ...
  while (i<N)
  {
	...
	if (accept)
	{
	  ...
	  // REPORT ON PROGRESS
	  //PRINT5("position progress = ",double(i+1)*100./double(N)," % with ave pot energy ",potential_energy/double(i+1)," ");
	}
  }
}
...
void ensemble::output_properties(int counter, double real_time)
{
...
void ensemble::output_positions(int i)
{
  // WRITE ONLY SPECIFIED ATOM
	   atoms[i].output_position();
}
